...
def test_pops_envelope_brackets_every_member_residual(pops_synth):
    """The paper's coverage object: at a member's own row the member reproduces
    its residual exactly, so the member min/max envelope must bracket it."""
    Sigma0, phi_t, r_t, c, phi_star, epi_var = pops_synth
    d = pops_corrections(Sigma0, phi_t, r_t, leverage_pct=0.0)
    lo, hi = pops_envelope(phi_t, d, chunk=7)       # odd chunk exercises the tail
    assert lo.shape == hi.shape == r_t.shape
    assert jnp.all(lo <= r_t + 1e-8) and jnp.all(r_t <= hi + 1e-8)
    assert jnp.all(hi - lo > 0)
