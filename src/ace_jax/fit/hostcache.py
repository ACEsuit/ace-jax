...
How.  With a pair-density feature map (--density pair) the residual GP only sees
the n_pair pair channels, whose inputs come from the pair radials alone
(rows.pair_feature_inputs).  The weighted linear rows w * Phi_B of every batch are
...
Memory: host ~ rows * L * 8 B (~90 GB for Cantor-3200 at L = 27.6k); device as
make_lml minus the per-evaluation ACE Jacobian, plus one chunk of rows.
"""
import jax
...
class HostCachedLML:
    ...
    def __init__(self, prob, ds, chunk=64):
        ...
        if P.shape[0] != cfg.D or np.any(P[:cfg.n_B] != 0):
            raise ValueError("HostCachedLML needs the pair-density feature map (--density pair): "
                             "with the full-descriptor map the residual rows need the ACE "
                             "Jacobian every evaluation, so caching the linear rows saves nothing")
        ...
        one = jax.jit(lambda b: (batch_linear_stats(prob.model, cfg, b),
                                 tuple(Rq * wq[:, None] for Rq, wq in
                                       zip(_flat(linear_rows(prob.model, cfg, b)[0]), _weights(b)))))
        lin = None
        for b in range(self.nb):
            st, Bw = one(jax.tree.map(lambda x: x[b], ds))
    ...
    # ------------------------------------------------------------------ pieces
    def _batch_stats(self, theta, batch, Bw, idx, valid):
        prob = self.prob
        U0, JU0 = pair_feature_inputs(prob.model, prob.ind, prob.cfg, batch)
    ...
    def _chunk_stats(self, a, xs):
        theta = from_array(a)
        dsc, Bw, idx, valid = xs
        f = jax.checkpoint(lambda th, b, B, i, v: self._batch_stats(th, b, B, i, v))
        ...
        return jax.lax.scan(body, zero, (dsc, Bw, idx, valid))[0]
    ...
    def _chunks(self):
        for s in range(0, self.nb, self.chunk):
            ...
            yield (dsc, tuple(jnp.asarray(r[s:e]) for r in self.rows),
                   ...
                   tuple(jnp.asarray(v[s:e]) for v in self.valid))
